...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# read outs, this will not work as I change the function to usse the bendix simulation approach
  alpha <- 0.05
  x <- NULL
  x <- t((res))
  x <- as.data.frame(x)
  mean( unlist(x[,"V1"]))                      # mean rate reduction (%)
  median( unlist(x[,"V2"]))                     # median Wilcoxon p-value
  table( unlist(x[,"V2"])< alpha)/sims                     # power mix model
  table( unlist(x[,"V3"])<alpha)/sims # Wilcoxon signed rank test power
  table( unlist(x[,"V4"])<alpha)/sims                    # ttest power
  table( unlist(x[,"V5"])<alpha)/sims           # ttest on ranks power
  # 
  mean( unlist(x[,"V6"]))   # intecept
  mean( unlist(x[,"V7"]))      # beta
  mean( unlist(x[,"V8"]))   # sd of diff
  
  # theory
  R <- seq(-.9,.9,.1)   # correlation
  X <- 10             # var of poisson (= mean)
  Y <- 7.5           # var of poisson (= mean)
  cbind(R,   sqrt(X+Y-(2*sqrt(X)*sqrt(Y)*R)))
