...
Use the inputs below to simulate an RCT with a count outcome measure. We estimate the treatment effect using a gamma Poisson model. Be careful with entering dispersion aka heterogeneity (ancillary) parameter. For example we may quote a negative binomial dispersion parameter of 0.8, referring to alpha, but we need to enter k or 1/alpha here so 1/0.8 = ~1.3 which is k
...
Show the equivalent parameterisation of gamma Poisson and negative binomial (rnbinom) ! Be careful with entering dispersion aka heterogeneity (ancillary) parameter. For example we may quote a negative binomial dispersion parameter of 0.8, referring to alpha, but we need to enter k or 1/alpha here so 1/0.8 = ~1.3 which is k
