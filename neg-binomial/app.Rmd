...
cat(paste0("This is an example of paired study design in which the outcome measure is counts of events and which does not use a concurrent control, so is not a great study design. \nNevertheless, we have ",input$N.p," subjects assessed before and after an intervention. \nThe Poisson model will be used, the observation time is the same for the the pre and post periods.\n"))
...
#  print(mean( unlist(x[,"V1"]))   )  
 ...
 # cat("Theoretical SD of differences\n")
  ...
  cat(paste0("Evaluations are based on the stated correlation, starting value "
...
A <- d[,1]
B <- d[,2]
before <- A + runif(length(d[,1]),-.2,.2)
after <-  B + runif(length(d[,2]),-.2,.2)
n <- length(before)
