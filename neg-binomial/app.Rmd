...
```{r, k or A}
  ...
  HTML(" <strong> With regard to negative binomial regression the variance can be written as </strong>")
                                       
  withMathJax(
                                           helpText('
                             $${{ }{{{m} + \\frac{m^2}{k}}}}\\!$$'))      
  
  HTML(" <strong>Sometimes this relationship is used</strong>")
  ...
  HTML(" <strong>And then the variance is expressed as</strong>")
  ...
  br()
  br()
  HTML(" <strong>In short, either the parameter k or its reciprocal alpha is referred to as the 'dispersion parameter' or 'shape parameter' or 'clustering coefficient' or 'heterogeneity' or 'heterogeneity (ancillary) parameter' or 'aggregation' parameter.  </strong>")
  br()
  br()
  HTML(" <strong> These two formulations can introduce confusion, so it is important to clarify if a dispersion value is k or alpha. R code below showing syntax.</strong>")
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# show how to use random generation for poisson to create neg binomial/gamma Poisson data
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  length <- 1        # not used here
  n      <- 100000   # very large n
  mu     <- exp(1)   # so expect coefficient on log scale of 1
  alpha  <- 0.001    # small alpha to approach Poisson
  k      <- 1/alpha  # 1000

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Gamma Poisson random generation and analysis
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  
  # Simulate the gamma-shaped subject effect
  # The larger the gamma shape parameter, the more dispersed is the count distribution.
  s <- rgamma(n, shape = 1/alpha, scale = alpha)
  
  # Simulate the counts
  lambda_1 <- mu * s

  y_0 <- rpois(n = n, lambda = lambda_1*length) 
  
  summary(MASS::glm.nb(y_0~1))

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Use rnbinom for random generation and then analyse
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  
  gp  <-  rnbinom(n = n,  prob=  1/(1 + mu*length* alpha),  size=1/alpha)
  
  summary(MASS::glm.nb(gp~1))

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Recover the expected mean and variance?
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  
  summary(y_0); summary(gp)
  var(y_0) ;var(gp)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  A  <-   MASS::rnegbin(n=n,   mu=mu,  theta=1/alpha)
  A1 <-   rnbinom(n=n,         mu=mu,  size= 1/alpha)
  B  <-   rpois(n=n , lambda=  mu * rgamma(n=n, shape = 1/alpha, scale = alpha))  
  B1 <-   rpois(n=n,  lambda= (mu * rgamma(n=n, shape = 1/alpha))*(alpha))  

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# examine means and variances, should be similar
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 
  z <- list(A,B,A1,B1)
  sapply(z, function(x) c(mean(x, na.rm = T), var(x, na.rm = T))) 
...
Show the equivalence of gamma Poisson and negative binomial (rnbinom) parmeterisations! Be careful entering dispersion aka heterogeneity (ancillary) parameter. For example, we may be quoted a negative binomial dispersion parameter of 0.8, referring to k, but we need to enter alpha or 1/k, so 1/0.8 = ~1.3 which is alpha.
...
### Gamma Poisson data simulation (Theta is synonymous with k = 1/$\alpha$)
...
### Negative binomial (rnbinom) data simulation (Theta is synonymous with k = 1/$\alpha$)
...
### Rootograms (rforge package so cannot be used in published Rshiny! )
...
```{r diagnostics1, eval=TRUE}
 ...
 f  <- r5()$model  # rnbinom approach simulation
   
 par(mfrow=c(1,2))
 # countreg::rootogram(f1, style = "hanging",     main = "Hanging plot based on gamma poisson simulation" ,col='red', fill="blue")
 # countreg::rootogram(f,  style = "hanging",     main = "Hanging plot based on rnbinom simulation", col='red', fill="blue")
...
Use the inputs below to simulate an RCT with a count outcome measure. We estimate the treatment effect using a gamma Poisson model.
...
```{r power canned}
      ...
      sliderInput('pl.k.can', 'Placebo dispersion (enter \u03B1 = 1/k)', value= 1.3,
                    min = 0, max = 5, step= .1, ticks=F)
       
      sliderInput('trt.k.can', 'Treatment dispersion (enter \u03B1 = 1/k)', value= 1.3,
                    min = 0, max = 5, step= .1, ticks=F)
...
### **We use canned formulae for sample size and allow comparison. The formulae don't account for follow up nor treatment discontinuation, so simulation is preferable. They are useful as a simulation sanity check and to ensure no confusion between k and $\\alpha$.**
...
reactive({
 ...
 cat(paste0("\nAn example analysis from the simulation is presented...remember Theta is synonymous with k or 1/alpha\n")) 
 
 print(res[2,][[1]])
 
 cat("\nFUNCTION 4 this one seems to not agree, perhaps it is the duration input? [16]\n") # 
 print(PASSED::power_NegativeBinomial( mu1 = mu0., mu2 = mu1., power=power.,
                                sig.level = 0.05, duration = 1, theta = k0,  #enter alpha
                                equal.sample = TRUE, alternative = c("two.sided" ),
                                approach = 1))
...
cat(paste0("\nWe report the average p-value from a mixed Poisson model and the power, Wilcoxon signed rank test power, T test power, T test on ranks power, intercept,\ntreatment effect and SD of differences. We also quote the theoretical SD of differences."))
...
reactive({
  ...
  cat(paste0("Evaluations based on ",input$sims.p," simulations take into consideration correlation, the default starting value is "
       ,input$r,". The observed mean correlation is ", formatz3(mean( unlist(x[,"V10"]))), "\nThe median p-value from a mixed effects Poisson model is "
       ,formatz3(median( unlist(x[,"V2"])))   , 
       ". \nPower based on the mixed model Wald test is ",
             
      formatz3((table( unlist(x[,"V2"])< alpha)/input$sims.p)[2][[1]]),
      
      "\nPower based on the Wilcoxon signed rank test power is ",
      formatz3((table( unlist(x[,"V3"])< alpha)/input$sims.p)[2][[1]]),
      "\nMean Hodges-Lehmann estimator ((1) Calc. post-pre 2) Calc. Walsh averages i.e (n+1)/2 pairwise averages 3) report median) ",
      ...
      formatz3((table( unlist(x[,"V4"])< alpha)/input$sims.p)[2][[1]]),
      "\nPower based on the t-test of ranks is ",
      formatz3((table( unlist(x[,"V5"])< alpha)/input$sims.p)[2][[1]]),
...
Wiki
====
**Notes: In general the app is quite slow as almost each page has some Monte Carlo simulation. But be patient eventually some information will appear.** The binomial distribution describes the number of r successes in n trials. The geometric distribution describes the number of failures before the first success, r. The gamma Poisson/negative binomial distribution describes the number of failures before the rth success. With respect to the gamma Poisson, the term ‘dispersion parameter’ confusingly can refer to either k or $\alpha$ ; other terms  include ‘shape parameter’ 'heterogeneity (ancillary) parameter' and ‘clustering coefficient’. The only restriction placed on $\alpha$ is that it take positive rational values, rarely above 4 (Hilbe's Negative Binomial regression book, page 190 [11]). Under dispersed data are assigned the minimum value of $\alpha$, corresponding to k going to infinity. The dispersion parameter k is commonly used as an inverse measure of aggregation in biological count data. The larger the gamma shape parameter, the more dispersed is the distribution. When $\alpha$ = 1, the negative binomial distribution takes the form of a
geometric distribution, which is the discrete correlate of the continuous negative exponential distribution [11]. The negative binomial distribution with $\alpha$ = 0 is Poisson. As the mean increases, the probability of a zero decreases, and the more the shape approximates a Gaussian distribution. As the Poisson mean gets larger it approaches normality, this is not the case for the negative binomial distribution. 
...
**k or $\alpha$** A warning!

**Gamma Poisson**

A demonstration showing negative binomial and gamma Poisson are one and the same. Using the same adjustable population parameters one data set is simulated using the R rnbinom function and another data set simulated using the gamma and Poisson distributions. The gamma Poisson is a mixture of different Poisson distributions in which the response counts are assumed to follow a mixed Poisson distribution with a shared gamma distributed random subject effect. The tabs at the bottom present some diagnostic plots. A lot of this is taken from reference [12].
