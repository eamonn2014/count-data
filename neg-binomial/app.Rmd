...
#     B=500
#    res <- replicate( simy, 
#                   nb.power.simp(n=B, k=1/k0, mu0=mu0, eff=mu1/mu0,  fup=1)) 
#      
#      mean(res<0.05)
...
# 1
    ...
    sliderInput('k', 'This needs to be k the heterogeneity (ancillary) parameter. k = 1/\u03B1', value = 1.3 , #c(10/13),
                  min = 0, max = 5 ,step=0.01, ticks=F)
...
# k1.=k0.
# Q1.=.5
# Q0.=.5 # Q0 = N0/N and Q1 = =N1/N


      sliderInput('P.can', 'Power', value=.9,
                    min = 0.5, max = 1, step=.01, ticks=F)
      ...
      sliderInput('pl.k.can', 'Placebo dispersion', value= 1/1.3,
                    min = 0, max = 4, step= .01, ticks=F)
       
      sliderInput('trt.k.can', 'Treatment dispersion', value= 1/1.3,
                    min = 0, max = 4, step= .01, ticks=F)
...
### We use canned formulae for sample size and allow comparison
...
reactive({
  ...
  B <- 2 * round(B/2) # need even number in each group for my sim functions
   ...
   cat(paste0("Sample size per group based on the user inputs and a function using equation 8 ",B," total patients are required (rounded up to an even number)\n"))
   
    tags$a("xxx", href = "https://bmcmedresmethodol.biomedcentral.com/track/pdf/10.1186/s12874-015-0023-0.pdf", target="_blank",
           tags$span(style="color:blue", "'Sample size calculations for skewed distributions (equation 8)"),) 
   
   
   
   cat("\n~~~~~~~~~~~~~~~~~~~~~~~~~\n") #2nd approach
   B1 <- skewsamp::n_negbinom(mean0 = mu0.,effect = 1-mu1./mu0., dispersion0 = k0.,dispersion1 = k1., q=Q0.,
                     alpha = type1, power = power.)
  
   cat(paste0("\nUsing R function skewsampl::n_negbinom whose R help quotes the above paper we obtain.\n\n")) 
   ...
   cat(paste0("\nHere is another formula from 'Keene, O. N., Jones, M. R. K., Lane, P. W., & Anderson, J. (2007). Analysis of exacerbation rates in asthma and chronic obstructive pulmonary disease: \nexample from the TRISTAN study. Pharmaceutical Statistics, 6(2), 89–97. doi:10.1002/pst.250 page 95,' \nthis gives an idea of the ball park subjects per treatment group,\nonly one dispersion parameter is allowed here and equal groups assumed, so 'Treatment dispersion' input will not work \n, so to compare this to other functions need to keep dispersion parameters equal. Also note dispersion is reciprocated here! ", ceiling(((mu0+mu1)/(mu0*mu1) +2*k0) * ((Ax+Bx)/(log(mu0/mu1)))^2)," \n"))
   
  cat("\n~~~~~~~~~~~~~~~~~~~~~~~~~\n")
  
    
     res <- replicate( simy, 
                  nb.power.simp(n=B/2, k=k0, mu0=mu0, eff=mu1/mu0,  fup=1)) 
...
```{r}
    ...
    tags$a(href = "https://www.amazon.co.uk/Discrete-Data-Analysis-Visualization-Categorical/dp/149872583X",target="_blank", tags$span(style="color:blue", "[12] Discrete Data Analysis with R: Visualization and Modeling Techniques for Categorical and Count Data, Michael Friendly, David Meyer"),) 
      div(p(" "))
    tags$a(href = "https://bmcmedresmethodol.biomedcentral.com/track/pdf/10.1186/s12874-015-0023-0.pdf",target="_blank", tags$span(style="color:blue", "[13] Sample size calculations for skewed distributions"),) 
