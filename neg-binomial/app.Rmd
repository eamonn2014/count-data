...
po.cor.power <- function(n=22, r=.75, mu0=10, eff.p=.75) { # r correlation mu0 placebo rate, mu1 expected change in rate
  ...
  wil <- wilcox.test(x=x1, paired=FALSE, correct=FALSE)$p.value
...
sliderInput('r', 'Correlation', value = c(.50),
...
cat(paste0("This example of paired study whose outcome measure is count of events and does not use a concurrent control, so is not a great study design. Nevertheless, we have ",input$N.p," subjects assessed before and after an intervention. \nThe Poisson model will be used, the observation time is the same for the the pre and post perioids.\n"))

cat(paste0("We will simulate the data based on a expected rate before the intervention of, ",input$mu0.p, " and an hypothesised effect of intervention ",input$eff.p,". \nWe also take into acount the correlation betwenn obervations for individuals, the starting value is" ,input$r,".\n")) 
...
cat(paste0("\n2) Transform them to correlated Uniform(0, 1) distributions with the normal CDF."))
cat(paste0("\n3) Transform them to any correlated probability distribution desired with that probability distribution’s inverse CDF!"))
...
#  print(mean( unlist(x[,"V1"]))   )  
 ...
 # cat("Theoretical SD of differences\n")
  ...
  cat(paste0("Evaluations are based on a stated correlation, starting value "
             ,input$r,"\nThe median p-value from a mixed effects Poisson model is "
             ,median( unlist(x[,"V2"]))   , 
             ". \nPower based on the mixed model is "
             ,input$eff.p,
             #"\nEvaluations are based on a stated correlation, starting value "
             #,input$r,
             
    #   table( unlist(x[,"V2"])< alpha)/input$sims.p)[1][[1]],
