...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  barplot(yb, xlim=c(0,high+2), ylab = "Percent", xlab="x", col='black', ylim=c(0,ymax),
          main = paste("Poisson \u03BC=", mu, ", based on common mean",sep=""),
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  barplot(yb, xlim=c(0,high+2), ylab = "Percent", xlab="x", col='red',
          ylim=c(0,ymax),
          main =  paste("Negative Binomial \u03BC=",mu," \u03B1=",alpha_a,", based on common mean and red curve alpha",sep=""),
          names.arg= as.character(xx))
...
When  $\alpha$>1, k is vanishingly small , k < 1 highly over dispersed ( $\alpha$ big).
...
sliderInput('mup', 'Common  \u03BC', value=1.5,
              min = 1, max = 25, step=.5, ticks=F)

sliderInput('alpha_ap', '\u03B1 for red curve', value = c(0.8),
              min = 0, max = 5 ,step=0.01, ticks=F)

sliderInput('alpha_bp', '\u03B1 for dark blue curve', value = c(0.8),
              min = 0, max = 5 ,step=0.01, ticks=F)

sliderInput('alpha_dp', '\u03B1 for purple curve', value = c(0.8),
              min = 0, max = 5 ,step=0.01, ticks=F)

sliderInput('simsp', 'Simulations (chart 2)', 50000,
              min = 50000, max = 500000,step=50000,ticks=F)

sliderInput('x_rangep', 'Plot x-range', value = c(0,10),
        min = 0, max = 80,step=5,ticks=F)

sliderInput('y_rangep', 'Plot y-range', value = c(0,.4),
        min = 0, max = 1,step=.05,ticks=F)
