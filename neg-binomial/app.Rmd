...
# 
# rnbinom simulation accounting for trt discontinuation, outputs p value
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
nb.power <- function(n=220, k=1.3, mu0=1, eff=0.65, drop1=.1, drop2=.1, fup=1) {
  ...
  mod <- MASS::glm.nb(y~dose+offset((logleng)))
  x <- summary(mod)
 
  # collect p-values
  p <-  x$coefficients["dosetrt","Pr(>|z|)"]
   
  return(list(p, y ,dose, x, mod, logleng))
...
# function to simulate negative binomial RCT
# 
# Poisson gamma simulation accounting for trt discontinuation, outputs p value
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
nb.power2 <- function(n=220, k=1.3, mu0=1, eff=0.65, drop1=.1, drop2=.1, fup=1) {
 ...
 # lambda1 <- mu   * s  ## Simulate rate
  ...
  # y <-   rpois(n = n*2, lambda = (length*lambda1))  +
  #        rpois(n = n*2, lambda = ((fup-length)*lambda1))  # ensure follow up here!
  
  y <-   rpois(n = n*2, lambda = (length*mu*s))  +
         rpois(n = n*2, lambda = ((fup-length)*mu0*s))  # ensure follow up here!
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if(input$perc) {
  x <-0:length(b) # work on getting % onto top of bars
  y1 <- yb + 0.02
  z <- round(yb,2) *100
  z <- paste0(z,"%")
...
>>>>>>> 74864954f33b590cbf5381242ecc0b4bc3100a9d
...
>>>>>>> 74864954f33b590cbf5381242ecc0b4bc3100a9d
 ...
 ##
  ...
  x <-as.numeric(colnames(nn))  # work on getting % onto top of bars
  y1 <- as.vector(nn)+4
 ...
 # count plot
  ...
  nn <- table(factor(trt))  # work on getting counts onto top of bars
  ...
  b <-barplot(table(factor(pla, levels=0:max(y)))  , main ="Placebo counts" , col='green',  ylim=c(0,u+25 ))
   ...
   nn <- table(factor(pla))  # work on getting counts onto top of bars
  x <-as.numeric(names(nn))
  y1 <- as.vector(nn)+10
  z <- as.character(as.vector(nn))
  text(x=b[x+1],y=(y1),labels=z, cex = 1)
